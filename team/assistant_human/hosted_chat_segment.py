...
def _current_team_anchor(
    ...
    try:
        ...
        raise _controller.ApiError(HTTPStatus.SERVICE_UNAVAILABLE, "Team identity could not be inspected") from exc
    if (
...
def _hosted_chat_setup(
    ...
    str,
    tuple[_controller._ActiveAssistant, ...],
    ...
    team_name = _controller._team_name_from_anchor(container)
    assistants = _controller._select_team_assistants(_controller._active_team_assistants(team_id), assistant_ids)
    files = _controller._chat_file_metadata(team_id, file_ids)
    try:
    ...
    api_key, generation = _controller._model_credential(owner, config.provider)
    _controller._require_model_credential_current(owner, config.provider, generation)
    identity = (
        container.id,
        owner,
        team_name,
...
def _raise_hosted_chat_problem(reason: str, exc: BaseException | None) -> NoReturn:
    if reason == "invalid-continuation" or reason == "invalid-suspension":
        raise _controller.ApiError(HTTPStatus.INTERNAL_SERVER_ERROR, f"invalid chat {reason.removeprefix('invalid-')}")
    ...
    if isinstance(exc, power_journal.PowerJournalError):
        raise _controller.ApiError(HTTPStatus.SERVICE_UNAVAILABLE, "Team Power execution state is unavailable") from exc
    ...
    if isinstance(exc, chat_orchestrator.ChatOrchestrationError):
        raise _controller.ApiError(HTTPStatus.BAD_GATEWAY, "Brain could not complete the Assistant turn") from exc
    if isinstance(exc, brain_runtime_client.BrainRuntimeError):
        raise _controller.ApiError(HTTPStatus.BAD_GATEWAY, "Brain runtime is unavailable") from exc
...
def _hosted_private_requirements(
    ...
    try:
        ...
        raise _controller.ApiError(HTTPStatus.CONFLICT, "Assistant secret contract is unavailable") from exc
...
def _hosted_approval_requirement(
    ...
    if len(interactions) != 1:
        raise _controller.ApiError(HTTPStatus.BAD_GATEWAY, "Assistant human approval request is invalid")
    ...
    try:
        requirement = assistant_approval_flow.requirement(
            ...
            _controller._hosted_power_identity(active)[1],
            len(answers),
        )
        granted = requirement.runs == "once" and _controller._assistant_approval_grants.is_granted(
            team_id,
        ...
        raise _controller.ApiError(HTTPStatus.BAD_GATEWAY, "Assistant human approval request is invalid") from exc
...
def _hosted_answer_log(
    ...
    if len(answers_by_interrupt) != len(answer_logs):
        raise _controller.ApiError(HTTPStatus.INTERNAL_SERVER_ERROR, "invalid chat answer log")
...
def _run_hosted_chat_segment(request: HostedChatSegmentRequest) -> chat_turn_engine.SegmentResult:
    ...
    answers_by_interrupt = _controller._hosted_answer_log(request.answer_logs)
    bindings: dict[str, _controller._ActiveAssistant] = {}
    ...
    def validate_power(assistant_id: str, power: str, power_input) -> object:
        return _controller._validate_assistant_power_input(bindings, assistant_id, power, power_input)
    ...
    def prepare() -> chat_turn_engine.PreparedSegment:
        ...
        batch = power_execution.PowerBatch(
            ...
            power_execution.PowerBatchStrategy(
                _controller._hosted_power_identity,
                execute_power,
                lambda request: _controller._require_hosted_power_rpc_envelope(
                    team_id,
                    bindings,
                    request,
                    answers_by_interrupt.get(request.interrupt_id, ()),
                ),
                lambda request: _controller._power_secret_generations(
                    team_id, bindings[request.assistant_id], request.power
                ),
                lambda request: _controller._power_account_generations(
                    team_id, bindings[request.assistant_id], request.power
                ),
            ),
        )
    ...
    def pause_for_private_inputs(
        ...
        requirements.accounts, requirements.secrets = _controller._hosted_private_requirements(
            team_id,
            bindings,
            requests,
        )
    ...
    while True:
        team_name, identity, outcome, requirements = chat_turn_engine.run_segment(
            chat_turn_engine.SegmentStrategy(
                runtime=_controller._brain_runtime,
                prepare=prepare,
                validate_power=validate_power,
                pause_for_private_inputs=pause_for_private_inputs,
                cancelled=lambda: _controller._token_cancelled(token),
                validate_context=validate_context,
                raise_problem=_controller._raise_hosted_chat_problem,
        ...
        requirement, granted = _controller._hosted_approval_requirement(
            team_id,
...
def _pause_hosted_chat(
    ...
    pending: _controller._PendingHostedChat,
) -> dict[str, object]:
    try:
        challenge = _controller._assistant_secret_challenges.create(team_id, requirements, pending)
    except assistant_secret_challenges.SecretChallengeError as exc:
        raise _controller.ApiError(HTTPStatus.CONFLICT, "Assistant secret request is already pending") from exc
    _controller._commit_hosted_suspension(team_id, token, outcome, pending, _controller._assistant_secret_challenges)
...
def _commit_hosted_suspension(
    ...
    chat_turn_engine.commit_suspension(
        ...
        lambda: _controller._commit_chat_terminal(team_id, token),
        lambda: challenge_store.cancel_team(team_id),
        lambda: _controller.ApiError(HTTPStatus.CONFLICT, "brain turn stopped"),
    )
...
def _hosted_account_challenge_payload(
    ...
    bindings: dict[str, _controller._HostedAssistantSecretBinding] = {}
    try:
        for requirement in challenge.requirements:
            assistant_id, contract, container = _controller._installed_assistant(
                challenge.team_id,
                requirement.assistant_id,
            )
            active = _controller._ActiveAssistant(assistant_id, contract, container)
            bindings[assistant_id] = _controller._HostedAssistantSecretBinding(_controller._hosted_secret_spec(active))
...
def _pause_hosted_connection(
    ...
    try:
        challenge = _controller._assistant_account_challenges.create(team_id, requirements, pending)
    except assistant_account_challenges.AccountChallengeError as exc:
        raise _controller.ApiError(HTTPStatus.CONFLICT, "Assistant account request is already pending") from exc
    _controller._commit_hosted_suspension(team_id, token, outcome, pending, _controller._assistant_account_challenges)
    return _controller._hosted_account_challenge_payload(challenge)
...
def _pause_hosted_input(
    ...
    if len(requirements) != 1:
        raise _controller.ApiError(HTTPStatus.INTERNAL_SERVER_ERROR, "invalid human input suspension")
    ...
    try:
        ...
        requirement = assistant_input_flow.requirement(
            interaction,
            _controller._hosted_power_identity(_controller._ActiveAssistant(assistant_id, contract, container))[1],
            len(answers),
        )
        challenge = _controller._assistant_input_challenges.create(team_id, requirement, pending)
    except (
        marketplace.MarketplaceError,
        assistant_input_challenges.InputChallengeError,
        assistant_input_flow.InputFlowError,
    ) as exc:
        raise _controller.ApiError(HTTPStatus.BAD_GATEWAY, "Assistant human input request is invalid") from exc
    _controller._commit_hosted_suspension(team_id, token, outcome, pending, _controller._assistant_input_challenges)
...
def _pause_hosted_approval(
    ...
    try:
        challenge = _controller._assistant_approval_challenges.create(team_id, requirements, pending)
    except assistant_approval_challenges.ApprovalChallengeError as exc:
        raise _controller.ApiError(HTTPStatus.CONFLICT, "Assistant approval is already pending") from exc
    _controller._commit_hosted_suspension(team_id, token, outcome, pending, _controller._assistant_approval_challenges)
...
def _hosted_segment_response(
    ...
    def pending(suspension: chat_orchestrator.ChatSuspension) -> _controller._PendingHostedChat:
        return _controller._PendingHostedChat(
    ...
    def complete(terminal: chat_orchestrator.ChatOutcome) -> dict[str, object]:
        if not _controller._commit_chat_terminal(team_id, token):
            raise _controller.ApiError(HTTPStatus.CONFLICT, "brain turn stopped")
        return {
            "team_id": team_id,
            "team_name": segment.team_name,
            "reply": terminal.reply[: _controller.CHAT_OUTPUT_CAP],
        }
    ...
    try:
        return chat_turn_engine.dispatch(
            ...
            pending,
            (
                lambda suspension, requirements, state: _controller._pause_hosted_connection(
                    team_id, token, suspension, requirements, state
                ),
                lambda suspension, requirements, state: _controller._pause_hosted_chat(
                    team_id, token, suspension, requirements, state
                ),
                lambda suspension, requirements, state: _controller._pause_hosted_input(
                    team_id, token, suspension, requirements, state
                ),
                lambda suspension, requirements, state: _controller._pause_hosted_approval(
                    team_id, token, suspension, requirements, state
                ),
            ),
            complete,
        ...
        raise _controller.ApiError(HTTPStatus.INTERNAL_SERVER_ERROR, str(exc)) from exc
